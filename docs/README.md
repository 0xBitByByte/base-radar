...
| [CLAUDE_EXECUTION_AUTHORITY.md](CLAUDE_EXECUTION_AUTHORITY.md) | **The permanent authority model and interruption policy** — who decides what (User/Claude/ChatGPT), the Master Execution Principle ("routine engineering work within an authorized task doesn't need re-confirmation"), exactly what Claude may do autonomously vs. must stop for, the strict Git policy (every mutating Git/GitHub operation requires explicit user authorization), PR-status granularity, and final-report requirements. Governs task-execution *authority/cadence*, distinct from CLAUDE_RULES.md (code), CLAUDE_PR_WORKFLOW.md (tracked-PR process), and CLAUDE_EFFICIENCY_STANDARD.md (tool-call efficiency). |
