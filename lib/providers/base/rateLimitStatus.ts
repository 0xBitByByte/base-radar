/**
 * Final Production Hardening PR — split out of `base/service.ts` for the
 * same reason as `blockscout/rateLimitStatus.ts`: `ProfileSources.tsx`'s
 * shared rate-limit lookup table needs this without pulling
 * `base/client.ts`'s real `fetch()` calls into any client bundle that
