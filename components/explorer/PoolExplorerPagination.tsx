 ...
 * WhaleExplorerPagination.tsx`'s exact pattern for the Pool Explorer's
 * own `PoolsQueryState`/`buildPoolsQuery`.
 */
...
type PoolExplorerPaginationProps = {
  /** This project's own Pools route (`/dashboard/projects/[slug]/pools`) — passed down rather than read via `usePathname()` so this stays a plain Server Component. */
  basePath: string;
...
export function PoolExplorerPagination({ basePath, state, currentPage, totalPages, hasPreviousPage, hasNextPage }: PoolExplorerPaginationProps) {
  if (totalPages <= 1) return null;

  const pageWindow = buildPageWindow(currentPage, totalPages);
  const hrefForPage = (page: number) => `${basePath}${buildPoolsQuery(state, { page })}`;

  return (
    <nav aria-label="Pools pagination" className="flex flex-wrap items-center justify-center gap-2 pt-2">
      {hasPreviousPage ? (
